...
unsigned char peek8(struct ufb_data * ufb,uint32_t offset){
	...
	regValue = SMI_READ_MMIO(ufb,offset);
...
void poke8(struct ufb_data * ufb,uint32_t offset,unsigned char value){	
    ...
    unsigned long regValue, mask, shift, new_offset;
    ...
    regValue = (regValue & ~mask) + (((unsigned long) value) << shift);
	new_offset = offset & ~0x00000003;
	SMI_WRITE_MMIO(ufb,new_offset,regValue);
...
int hw_set_mode(const struct sm750fb_par * par,const struct fb_var_screeninfo *var,const struct fb_fix_screeninfo * fix)
{
	...
	*/
    ...
    {
	        //dbg_msg("SET DispOutput: use PANEL_CRT_DUAL");
            ret = ddk750_setLogicalDispOutput(ufb,PANEL_CRT_DUAL,0);
    }else if(par->flagofmirror ==2 )
    {
            //dbg_msg("SET DispOutput: use PANEL_CRT_SIMUL");
...
#if 1
{
	...
	/* sanity check */
#ifdef _USE_RANDR_
	if(area.x < 0 || area.x > info->var.xres_virtual*2 - 1) //change tmp. 2013-8-22. ilena
		//LEAVE(-EINVAL);
	    return (-EINVAL);
	if(area.y < 0 || area.y > info->var.yres_virtual - 1)
		//LEAVE(-EINVAL);
        return (-EINVAL);
	if(area.width < 1 || area.width > info->var.xres_virtual*2)
		//LEAVE(-EINVAL);
	    return (-EINVAL);
	if(area.height < 1 || area.height > info->var.yres_virtual*2)
		//LEAVE(-EINVAL);
        return (-EINVAL);
#else	
