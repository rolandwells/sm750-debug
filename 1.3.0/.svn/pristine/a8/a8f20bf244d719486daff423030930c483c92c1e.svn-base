...
byte I2CReadBlock(struct i2c_client *client, byte RegAddr, byte NBytes, byte * Data )
{
	byte i = 0;
	while(i<NBytes){
		Data[i] = i2cReadReg(g_ufb,SII9022A_I2C_ADDRESS,RegAddr+i);
		i++;
	}
	return IIC_OK;
}
...
byte I2CWriteBlock( struct i2c_client *client, byte RegAddr, byte NBytes, byte * Data )
{
	byte i = 0;
	while(i < NBytes){
		i2cWriteReg(g_ufb,SII9022A_I2C_ADDRESS,RegAddr+i,Data[i]);
		i++;
	}
	return IIC_OK;
}
...
{
	return i2cReadReg(g_ufb,SII9022A_I2C_ADDRESS,	RegOffset);
}

void WriteByteTPI (byte RegOffset, byte Data)
{
	i2cWriteReg(g_ufb,SII9022A_I2C_ADDRESS,RegOffset,Data);
}
...
void siHdmiTx_VideoSel(enum avidmode vmode)
{
...
//	siHdmiTx.SyncMode 			= 1;
	switch (vmode)
	{
		...
		case HDMI_1280_960_60:
			siHdmiTx.VIC 		= PC_BASE + 23;
			siHdmiTx.AspectRatio 	= VMD_ASPECT_RATIO_4x3;
			siHdmiTx.Colorimetry	= COLORIMETRY_709;
			siHdmiTx.TclkSel		= X1;
			break;

		case HDMI_1400_1050_60:
			siHdmiTx.VIC 		= PC_BASE + 30;
			siHdmiTx.AspectRatio 	= VMD_ASPECT_RATIO_4x3;
			siHdmiTx.Colorimetry	= COLORIMETRY_709;
			siHdmiTx.TclkSel		= X1;
			break;

		case HDMI_1600_1200_60:
			siHdmiTx.VIC 		= PC_BASE + 33;
			siHdmiTx.AspectRatio 	= VMD_ASPECT_RATIO_4x3;
			siHdmiTx.Colorimetry	= COLORIMETRY_709;
			siHdmiTx.TclkSel		= X1;
			break;
