 ...
 */
void panelWaitVerticalSync(void *ufb,unsigned long vsync_count)
{
    unsigned long status;
    
    /* Do not wait when the Panel PLL is off or display control is already off. 
       This will prevent the software to wait forever. */
    if ((FIELD_GET(peek32(ufb,PANEL_PLL_CTRL), PANEL_PLL_CTRL, POWER) ==
         PANEL_PLL_CTRL_POWER_OFF) ||
        (FIELD_GET(peek32(ufb,PANEL_DISPLAY_CTRL), PANEL_DISPLAY_CTRL, TIMING) ==
    ...
    while (vsync_count-- > 0)
    {
        ...
        do
        {
            status = FIELD_GET(peek32(ufb,SYSTEM_CTRL),
                               SYSTEM_CTRL,
                               PANEL_VSYNC);
        }
 ...
 *
 */
void swPanelPowerSequence(void * ufb,disp_state_t dispState, unsigned long vsync_delay)
{
    unsigned long panelControl = peek32(ufb,PANEL_DISPLAY_CTRL);
    if (dispState == DISP_ON)
    {
        /* Turn on FPVDDEN. */
        panelControl = FIELD_SET(panelControl,
                     PANEL_DISPLAY_CTRL, FPVDDEN, HIGH);
        poke32(ufb,PANEL_DISPLAY_CTRL, panelControl);
        panelWaitVerticalSync(ufb,vsync_delay);
 ...
 *      +-----------+-----------+-----------------------------------------------+
 */
void setDisplayControl(void * ufb,disp_control_t dispControl, disp_state_t dispState)
{
    unsigned long ulDisplayCtrlReg, ulReservedBits;

    if (dispControl == PANEL_CTRL)
    {
        ulDisplayCtrlReg = peek32(ufb,PANEL_DISPLAY_CTRL);

        //DDKDEBUGPRINT((DISPLAY_LEVEL, "(setDispCtrl) PANEL_DISPLAY_CTRL before set: %x\n", peek32(ufb,PANEL_DISPLAY_CTRL)));
        ...
        if (dispState == DISP_ON)
        {
            ...
            poke32(ufb,PANEL_DISPLAY_CTRL, ulDisplayCtrlReg);
            ...
            {
                poke32(ufb,PANEL_DISPLAY_CTRL, ulDisplayCtrlReg);
            } while((peek32(ufb,PANEL_DISPLAY_CTRL) & ~ulReservedBits) != (ulDisplayCtrlReg & ~ulReservedBits));
        }
        else
        {
    ...
    {
        ulDisplayCtrlReg = peek32(ufb,CRT_DISPLAY_CTRL);

        //DDKDEBUGPRINT((DISPLAY_LEVEL, "(setDispCtrl) CRT_DISPLAY_CTRL before set: %x\n", peek32(ufb,CRT_DISPLAY_CTRL)));
        ...
        if (dispState == DISP_ON)
        {
            ...
            poke32(ufb,CRT_DISPLAY_CTRL, ulDisplayCtrlReg);
...
)
{
    ...
    {
        control = peek32(ufb,PANEL_DISPLAY_CTRL);
        if (dispState == DISP_ON)
        {
#if 0        
        ...
        t = 0;
        while(peek32(ufb,PANEL_DISPLAY_CTRL) != control && t<99)
        {
            poke32(ufb,PANEL_DISPLAY_CTRL, control);
            t++;
        }
        //inf_msg("After tried %d times,Read 0x80000 == %08x\n",t,peek32(ufb,0x80000));
    }
    else    /* CRT Path */
    {
        control = peek32(ufb,CRT_DISPLAY_CTRL);
        ...
        poke32(ufb,CRT_DISPLAY_CTRL, control);
    }
}
...
)
{
    ...
    switch (output)
    {
        ...
        {        
            	/* 
            		In here, all the display device has to be turned off first before the
               		the display control. 
               	*/
            	setDisplayControl(ufb,PANEL_CTRL, DISP_OFF);    /* Turn off Panel control */
            	setDisplayControl(ufb,CRT_CTRL, DISP_OFF);      /* Turn off CRT control */
	        setDisplayControl(ufb,VGA_CTRL, DISP_OFF);      /* Turn off VGA control */
               
            	swPanelPowerSequence(ufb,DISP_OFF, 4);          /* Turn off Panel */
            	setDAC(ufb,DISP_OFF);                           /* Turn off DAC */
            	ddk750_setDPMS(ufb,DPMS_OFF);                          /* Turn off DPMS */            
            	setDualPanel(ufb,0);                            /* Turn off Panel 2 */    			
			
            	setPath(ufb,PANEL_PATH, PANEL_CTRL, DISP_OFF);  /* Turn off Panel path */
            	setPath(ufb,CRT_PATH, PANEL_CTRL, DISP_OFF);    /* Turn off CRT path */
            	break;
        }    
        case PANEL1_ONLY:
        {         
		/*PANEL1 only	(24 or 36 bit TFT) show primary controller data	*/
            	setDisplayControl(ufb,PANEL_CTRL, DISP_ON);     /* Turn on Panel control */            	
            	setDisplayControl(ufb,CRT_CTRL, DISP_OFF);  /* Turn off CRT control */ 
		setDisplayControl(ufb,VGA_CTRL, DISP_OFF);      /* Turn off VGA control */

		swPanelPowerSequence(ufb,DISP_ON, 4);           /* Turn on Panel */
           	setDAC(ufb,DISP_OFF);                           /* Turn off DAC */		
            	ddk750_setDPMS(ufb,DPMS_OFF);                          /* Turn off DPMS */            
            	setDualPanel(ufb,0);                            /* Turn off Panel 2 */            
            
            	setPath(ufb,PANEL_PATH, PANEL_CTRL, DISP_ON);   /* Turn on Panel Path and use panel data */
            	setPath(ufb,CRT_PATH, CRT_CTRL, DISP_ON);    /* Turn CRT path to secondary controller */            
            	break;
        }    
		...
		poke32(ufb,PANEL_DISPLAY_CTRL,FIELD_SET(peek32(ufb,PANEL_DISPLAY_CTRL),
							PANEL_DISPLAY_CTRL,DATA,ENABLE));
		
            	setDAC(ufb,DISP_ON);                            /* Turn on DAC */
            	ddk750_setDPMS(ufb,DPMS_ON);                           /* Turn on DPMS to drive CRT */				
            	setDualPanel(ufb,0);                            /* Turn off Panel 2 */           
				
            	setPath(ufb,PANEL_PATH, PANEL_CTRL, DISP_ON);  /* Turn off Panel Path */
            	setPath(ufb,CRT_PATH, PANEL_CTRL, DISP_ON);     /* Turn off CRT Path and use panel data */
            	break;
        }
		case PANEL_2_PRIMARY:
		{
            	setDisplayControl(ufb,PANEL_CTRL, DISP_ON);     /* Turn on Panel Control */
            	swPanelPowerSequence(ufb,DISP_ON, 4);           /* Turn on Panel */		
            	setPath(ufb,PANEL_PATH, PANEL_CTRL, DISP_ON);   /* Turn on Panel Path and use panel data */
		...
		case CRT_2_SECONDARY:
		{
            	setDisplayControl(ufb,CRT_CTRL, DISP_ON);       /* Turn on CRT control */
            	setDAC(ufb,DISP_ON);                            /* Turn on DAC */
            	ddk750_setDPMS(ufb,DPMS_ON);                           /* Turn on DPMS to drive CRT */            
//            	setDualPanel(ufb,0);                            			/* Turn off Panel 2 */
				setPath(ufb,CRT_PATH, CRT_CTRL, DISP_ON);       /* Turn on CRT Path and use CRT data */
			break;
		}
...
default_output_flag:			
		...
		*/
            	...
            	setPath(ufb,PANEL_PATH, PANEL_CTRL, DISP_ON);   /* Turn on Panel Path and use panel data */
            	setPath(ufb,CRT_PATH, PANEL_CTRL, DISP_ON);     /* Turn on CRT Path and use panel data */            
            	break;
        }    
	case PANEL_CRT_SIMUL_SEC:
	{
		/*	like above but show secondary controller data	*/
            	...
            	setPath(ufb,PANEL_PATH, CRT_CTRL, DISP_ON);   /* Turn ON Panel Path and use secondary ctrl 	*/
            	setPath(ufb,CRT_PATH, CRT_CTRL, DISP_ON);     /* Turn on CRT Path and use secondary ctrl	 */            
            	break;		
	}
	case PANEL1_PANEL2_SIMUL:
	{
		/*	dual 18 bit TFT show primary controller data	*/
            	...
            	swPanelPowerSequence(ufb,DISP_ON, 4);           /* Turn on Panel */			
            	setDAC(ufb,DISP_OFF);                            /* Turn on DAC */
            	ddk750_setDPMS(ufb,DPMS_OFF);                           /* Turn on DPMS to drive CRT */          
		setPanelType(ufb,TFT_18BIT);
            
            	setPath(ufb,PANEL_PATH, PANEL_CTRL, DISP_ON);   /* Turn ON Panel Path and use secondary ctrl 	*/
            	setPath(ufb,CRT_PATH, PANEL_CTRL, DISP_ON);     /* Turn on CRT Path and use secondary ctrl	 */            
            	break;
	}
        case PANEL_CRT_DUAL: 
        {
		...
		*/
            	...
            	if (swapDisplay == 1)
            	{                        		
                	setPath(ufb,PANEL_PATH, CRT_CTRL, DISP_ON);     /* Turn on Panel Path and use CRT data */
                	setPath(ufb,CRT_PATH, PANEL_CTRL, DISP_ON);     /* Turn on CRT Path and use Panel data */              
            	}
            	else
            	{
...
)
{
    ...
    value = peek32(ufb,PANEL_DISPLAY_CTRL);
    if (enable == 1)
    ...
    poke32(ufb,PANEL_DISPLAY_CTRL, value);
    
    /* Force to 18-bit Panel. Do this after enable Dual Panel function above
       since setPanelType(ufb,TFT_18BIT) will check if the Dual Panel is enable or
 ...
 *      1   - Enable  
 */
unsigned char isDualPanelEnable(void * ufb)
{
    unsigned long value;
    value = FIELD_GET(peek32(ufb,PANEL_DISPLAY_CTRL), PANEL_DISPLAY_CTRL, DUAL_DISPLAY);    
 ...
 *      -1   - Fail 
 */
long ddk750_initDisplay(void * ufb)
{
    ...
    if ((dviInit(
        ...
        ) != 0) && (dviGetVendorID(ufb) != 0x0000) && (dviGetDeviceID(ufb) != 0x0000))
    {
        return (-1);
    }
