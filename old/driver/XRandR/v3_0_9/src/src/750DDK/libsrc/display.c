 ...
 *      1   - Enable  
 */
unsigned char isDualPanelEnable()
{
    unsigned long value;
 ...
 */
_X_EXPORT void swPanelPowerSequence(disp_state_t dispState, unsigned long vsync_delay)
{
 ...
 * Input: On or off
 */
_X_EXPORT void setDAC(disp_state_t state)
{
...
void setDisplayControl(disp_control_t dispControl, disp_state_t dispState)
{
    ...
    if (dispControl == PRIMARY_CTRL)
    {
        ...
        if (dispState == DISP_ON)
        {
            ...
            {
                pokeRegisterDWord(PRIMARY_DISPLAY_CTRL, ulDisplayCtrlReg);
            } while((peekRegisterDWord(PRIMARY_DISPLAY_CTRL) & ~ulReservedBits) != (ulDisplayCtrlReg & ~ulReservedBits));
        }
        else
        {
    ...
    else
    {
        ...
        if (dispState == DISP_ON)
        {
            ...
            {
                pokeRegisterDWord(SECONDARY_DISPLAY_CTRL, ulDisplayCtrlReg);
            } while((peekRegisterDWord(SECONDARY_DISPLAY_CTRL) & ~ulReservedBits) != (ulDisplayCtrlReg & ~ulReservedBits));
        }
        else
        {
            /* When turning off, there is no rule on the programming sequence since whenever the
