...
)
{
    ...
    {
        /* CRT Display: CRT_PLL */
        pokeRegisterDWord(CRT_PLL_CTRL, formatPllReg(pPLL)); 
    ...
    {		
        ...
        pokeRegisterDWord(PANEL_PLL_CTRL, formatPllReg(pPLL));
        ...
        if (pPLL->clockType == PANEL_PLL)
        {
            ...
            pokeRegisterDWord(VGA_CONFIGURATION, 
                FIELD_SET(peekRegisterDWord(VGA_CONFIGURATION), VGA_CONFIGURATION, PLL, PANEL));
#endif
        }
        ...
        pokeRegisterDWord(PANEL_HORIZONTAL_TOTAL,ulReg=
        ...
        pokeRegisterDWord(PANEL_HORIZONTAL_SYNC,ulReg=
        ...
        pokeRegisterDWord(PANEL_VERTICAL_TOTAL,ulReg=
        ...
        pokeRegisterDWord(PANEL_VERTICAL_SYNC,ulReg=
	...
	ulTmpValue = peekRegisterDWord(PANEL_DISPLAY_CTRL);	
	...
	do
	{
		pokeRegisterDWord(PANEL_DISPLAY_CTRL,B);
		A = peekRegisterDWord(PANEL_DISPLAY_CTRL) & ~ulReservedBits;
		t++;
	}while(A!=B && t<10); 
    }
}
...
long setCustomModeEx(logicalMode_t *pLogicalMode, mode_parameter_t *pUserModeParam)
{
    ...
    /* Set the mode first */
    returnValue = ddk750_setCustomMode(pLogicalMode, pUserModeParam);
    if (returnValue == 0)
    {
...
{
    unsigned long value = 0, adjustedHorzTotal, adjustedPriFrequency, adjustedSecFrequency;
    unsigned long vertScaleFactor, horzScaleFactor, dstWidth, dstHeight;
        ...
        (pLogicalMode->y < pLogicalMode->yLCD))     
    {
        ...
        if (isDisplayControlSwapped() == 0)
        {
            ...
            if (pLogicalMode->dispCtrl == PANEL_CTRL)
            {
                ...
                value = peekRegisterDWord(CRT_VERTICAL_EXPANSION);
                ...
                pokeRegisterDWord(CRT_VERTICAL_EXPANSION, value);
                ...
                pokeRegisterDWord(CRT_HORIZONTAL_EXPANSION, value);            
        ...
        #endif
                value = peekRegisterDWord(CRT_HORIZONTAL_TOTAL);
                value = FIELD_VALUE(value, CRT_HORIZONTAL_TOTAL, TOTAL, adjustedHorzTotal); 
                pokeRegisterDWord(CRT_HORIZONTAL_TOTAL, value);
    #else            
                /* 
                 * srcHT = ((dstHT * srcPixelClock) / dstPixelClock) * (dstVDE / srcVDE) 
                 */
        ...
        #endif
                value = peekRegisterDWord(PANEL_HORIZONTAL_TOTAL);
	...
	if(pLogicalMode->bpp != 8)
        	displaySetInterpolation(1, 1);
	else
		displaySetInterpolation(0, 0);
    }
    else
    {
