...
static uint32_t gDebugLevelMask = 0;
 ...
 */
void ddkDebugPrintInit(ddk_debug_output_t debugOutput, uint32_t debugLevelMask)
{
 ...
 */
void ddkDebugPrint(uint32_t debugLevel, const char* pszFormat, ...)
{
    ...
    if ((debugLevel & gDebugLevelMask) != 0)
    {
	    /* Format the string */
	    va_list arg_ptr;
	    va_start(arg_ptr, pszFormat);
	    nWritten = (uint32_t) vsnprintf(pszPrintBuffer, BUFFER_LENGTH - 1, pszFormat, arg_ptr);
	    va_end(arg_ptr);
        
        /* Check for buffer overflow */
	    if (nWritten == (uint32_t)(-1))
	    {
